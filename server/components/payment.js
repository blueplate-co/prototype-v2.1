...
Meteor.methods({
  ...
  "payment.createSource"(Stripetoken) {
    ...
    var createSource = Meteor.wrapAsync(
      stripe.customers.createSource.bind(stripe.customers)
    );
    var stripeUserID = Meteor.users.find({ _id: Meteor.userId() }).fetch()[0]
      .stripe_id;
  ...
  "payment.depositCredits"(creditPackage, buyer_id) {
    if (creditPackage == "") {
      ...
      var buyerEmail = Meteor.users.findOne({ _id: buyer_id }).emails[0]
        .address;
      var credits = 0;
      var ammount = 0;
